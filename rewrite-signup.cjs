...
const leftSide = parts[0] + splitMarker + '\n';

const rightSide = `
  <div class="w-full md:w-1/2 h-screen flex flex-col p-4 sm:p-8 relative bg-white overflow-hidden" style="view-transition-name: auth-right;">
    
    <!-- Mobile Logo -->
    <div class="md:hidden flex items-center justify-center w-12 h-12 mb-4 shrink-0">
    ...
    <div class="w-full max-w-lg mx-auto flex items-center justify-between mb-4 shrink-0">
      ...
      <div class="flex items-center gap-3">
        ...
        <span id="stepText" class="text-xs font-bold text-gray-500 uppercase tracking-widest transition-all">Step 1 of 4</span>
      </div>
    </div>

    <!-- Signup Container -->
    <div class="w-full max-w-lg mx-auto relative pb-20 h-full overflow-y-auto hide-scrollbar custom-scrollbar relative">
      
      <form id="signupForm" class="w-full pb-10" onsubmit="handleGenericForm(event)" novalidate>
        
        <!-- STEP 1: Role Selection -->
        <div id="step1" class="w-full step-container">
          ...
          <!-- Grid layout adapted for 50% screen width (2 columns instead of 5) -->
          ...
          <button type="button" onclick="goToStep(1, 2)" class="w-full bg-slate-950 text-white py-3 rounded-xl font-bold text-sm hover:bg-slate-800 hover:scale-[1.02] hover:shadow-lg transition-all duration-300 flex items-center justify-center gap-2 group mt-4">
        ...
        <!-- STEP 2: Personal Details & Dynamic Role Info -->
        <div id="step2" class="w-full step-container step-hidden-right absolute top-0 left-0">
          ...
          <div class="flex flex-col gap-4 w-full">
            ...
            <div class="bg-white rounded-[20px] p-4 shadow-sm border border-gray-100 hover:border-gray-200 transition-colors">
              <h3 class="text-sm font-bold text-slate-950 flex items-center gap-2 mb-4"><i data-lucide="user" class="w-4 h-4 text-red-600"></i> Personal Details</h3>
              <div class="flex flex-col gap-3">
                <div class="flex flex-col gap-2 relative">
                  <label class="text-[10px] font-bold text-gray-500 uppercase tracking-wider">Full Name *</label>
                  <input type="text" id="f_name" class="w-full bg-gray-50 rounded-xl px-3 py-2 border border-gray-100 outline-none focus:border-slate-950 focus:ring-4 focus:ring-slate-950/5 focus:bg-white transition-all text-slate-950 font-bold text-sm" placeholder="e.g. Rajesh Kumar" required>
                </div>
                ...
                <div class="flex flex-col gap-2 relative">
                  ...
                  <div class="flex bg-gray-50 rounded-xl border border-gray-100 focus-within:border-slate-950 focus-within:ring-4 focus-within:ring-slate-950/5 focus-within:bg-white transition-all overflow-hidden">
                    <span class="px-3 py-2 border-r border-gray-100 font-bold text-gray-500 text-xs flex items-center gap-1.5">IN +91</span>
                    <input type="tel" id="f_phone" class="flex-1 px-3 py-2 bg-transparent outline-none font-bold text-slate-950 text-sm" placeholder="98765 43210" maxlength="10" required>
                  </div>
                </div>
                ...
                <div class="flex flex-col gap-2 relative">
                  ...
                  <div class="flex bg-gray-50 rounded-xl border border-gray-100 focus-within:border-slate-950 focus-within:ring-4 focus-within:ring-slate-950/5 focus-within:bg-white transition-all overflow-hidden">
                    <span class="px-3 py-2 border-r border-gray-100 font-bold text-gray-500 text-xs flex items-center gap-1.5">IN +91</span>
                    <input type="tel" id="f_wa" class="flex-1 px-3 py-2 bg-transparent outline-none font-bold text-slate-950 text-sm" placeholder="WhatsApp Number" maxlength="10">
                  </div>
                </div>

                <div class="flex flex-col gap-2 relative">
                  <label class="text-[10px] font-bold text-gray-500 uppercase tracking-wider">Email Address *</label>
                  <input type="email" id="f_email" class="w-full bg-gray-50 rounded-xl px-3 py-2 border border-gray-100 outline-none focus:border-slate-950 focus:ring-4 focus:ring-slate-950/5 focus:bg-white transition-all text-slate-950 font-bold text-sm" placeholder="you@example.com" required>
                </div>
              </div>
            </div>

            <!-- Dynamic Block: NAR Realtor -->
            <div id="cond_nar" class="bg-blue-50/50 rounded-[20px] p-4 shadow-sm border border-blue-100 hidden">
              ...
              <div class="flex flex-col gap-3">
                <div class="flex flex-col gap-2 relative">
                  ...
                  <input type="url" id="f_website" class="w-full bg-white rounded-xl px-3 py-2 border border-blue-200 outline-none focus:border-blue-600 focus:ring-4 focus:ring-blue-600/10 transition-all text-slate-950 font-bold text-sm" placeholder="https://www.yoursite.com">
                </div>
                <div class="flex flex-col gap-2 relative">
                  <label class="text-[10px] font-bold text-gray-500 uppercase tracking-wider">NAR Member ID</label>
                  <input type="text" id="f_nar_id" class="w-full bg-white rounded-xl px-3 py-2 border border-blue-200 outline-none focus:border-blue-600 focus:ring-4 focus:ring-blue-600/10 transition-all text-slate-950 font-bold text-sm uppercase" placeholder="NAR-XXXXXXXX">
                </div>
              </div>
            </div>

            <!-- Dynamic Block: Channel Partner -->
            <div id="cond_cp" class="bg-indigo-50/50 rounded-[20px] p-4 shadow-sm border border-indigo-100 hidden">
              ...
              <div class="flex flex-col gap-3">
                <div class="flex flex-col gap-2 relative">
                  ...
                  <div class="relative">
                    <select id="cp_rera_reg" class="w-full bg-white rounded-xl px-3 py-2 border border-indigo-200 outline-none focus:border-indigo-600 focus:ring-4 focus:ring-indigo-600/10 transition-all text-slate-950 font-bold text-sm appearance-none cursor-pointer">
                ...
                <div class="flex flex-col gap-2 relative">
                  ...
                  <input type="text" id="cp_rera_no" class="w-full bg-white rounded-xl px-3 py-2 border border-indigo-200 outline-none focus:border-indigo-600 focus:ring-4 focus:ring-indigo-600/10 transition-all text-slate-950 font-bold text-sm uppercase" placeholder="KRERA-XXXX-00000">
                </div>
              </div>
            </div>

            <!-- Dynamic Block: Builder -->
            <div id="cond_builder" class="bg-emerald-50/50 rounded-[20px] p-4 shadow-sm border border-emerald-100 hidden">
              ...
              <div class="flex flex-col gap-3">
                <div class="flex flex-col gap-2 relative">
                  ...
                  <input type="text" id="bl_name" class="w-full bg-white rounded-xl px-3 py-2 border border-emerald-200 outline-none focus:border-emerald-600 focus:ring-4 focus:ring-emerald-600/10 transition-all text-slate-950 font-bold text-sm" placeholder="Company / Developer Name">
                </div>
                <div class="grid grid-cols-2 gap-3">
                  <div class="flex flex-col gap-2 relative">
                    <label class="text-[10px] font-bold text-gray-500 uppercase tracking-wider">Primary Contact</label>
                    <input type="text" id="bl_pc_name" class="w-full bg-white rounded-xl px-3 py-2 border border-emerald-200 outline-none focus:border-emerald-600 focus:ring-4 focus:ring-emerald-600/10 transition-all text-slate-950 font-bold text-sm" placeholder="Name">
                  </div>
                  <div class="flex flex-col gap-2 relative">
                    <label class="text-[10px] font-bold text-gray-500 uppercase tracking-wider">Number</label>
                    <input type="tel" id="bl_pc_ph" class="w-full bg-white rounded-xl px-3 py-2 border border-emerald-200 outline-none focus:border-emerald-600 focus:ring-4 focus:ring-emerald-600/10 transition-all text-slate-950 font-bold text-sm" placeholder="Phone">
                  </div>
                </div>
                <div class="flex flex-col gap-2 relative">
                  <label class="text-[10px] font-bold text-gray-500 uppercase tracking-wider">Project Name</label>
                  <textarea id="bl_project" class="w-full bg-white rounded-xl px-3 py-2 border border-emerald-200 outline-none focus:border-emerald-600 focus:ring-4 focus:ring-emerald-600/10 transition-all text-slate-950 font-medium text-sm min-h-[50px]" placeholder="List your current / upcoming project names..."></textarea>
                </div>
              </div>
            </div>

            <button type="button" onclick="goToStep(2, 3)" class="w-full bg-slate-950 text-white py-3 mt-2 rounded-xl font-bold text-sm hover:bg-slate-800 hover:scale-[1.02] hover:shadow-lg transition-all duration-300 flex items-center justify-center gap-2 group">
              Continue <i data-lucide="arrow-right" class="w-4 h-4 transition-transform group-hover:translate-x-1"></i>
            </button>
          </div>
        </div>
        ...
        <div id="step3" class="w-full step-container step-hidden-right absolute top-0 left-0">
          <div class="flex items-center gap-3 mb-4">
          ...
          <div class="bg-white rounded-[20px] p-4 shadow-sm border border-gray-100 hover:border-gray-200 transition-colors">
            ...
            <div class="flex flex-col gap-3">
              <div class="flex flex-col gap-2 relative">
                ...
                <textarea id="f_address" class="w-full bg-gray-50 rounded-xl px-3 py-2 border border-gray-100 outline-none focus:border-slate-950 focus:ring-4 focus:ring-slate-950/5 focus:bg-white transition-all text-slate-950 font-medium text-sm min-h-[60px]" placeholder="House / Flat No, Street, Area..."></textarea>
              </div>

              <div class="grid grid-cols-2 gap-3">
                <div class="flex flex-col gap-2 relative">
                  <label class="text-[10px] font-bold text-gray-500 uppercase tracking-wider">City</label>
                  <input type="text" id="f_city" class="w-full bg-gray-50 rounded-xl px-3 py-2 border border-gray-100 outline-none focus:border-slate-950 focus:ring-4 focus:ring-slate-950/5 focus:bg-white transition-all text-slate-950 font-bold text-sm" placeholder="e.g. Kozhikode">
                </div>
                <div class="flex flex-col gap-2 relative">
                  <label class="text-[10px] font-bold text-gray-500 uppercase tracking-wider">PIN Code</label>
                  <input type="text" id="f_pin" class="w-full bg-gray-50 rounded-xl px-3 py-2 border border-gray-100 outline-none focus:border-slate-950 focus:ring-4 focus:ring-slate-950/5 focus:bg-white transition-all text-slate-950 font-bold text-sm uppercase" placeholder="673001" maxlength="6">
                </div>
              </div>
              ...
              <div class="flex flex-col gap-2 relative">
                ...
                <div class="relative">
                  <select id="f_state" class="w-full bg-gray-50 rounded-xl px-3 py-2 border border-gray-100 outline-none focus:border-slate-950 focus:ring-4 focus:ring-slate-950/5 focus:bg-white transition-all text-slate-950 font-bold text-sm appearance-none cursor-pointer">
              ...
              <div class="flex flex-col gap-2 relative">
                ...
                <input type="text" id="f_area" class="w-full bg-gray-50 rounded-xl px-3 py-2 border border-gray-100 outline-none focus:border-slate-950 focus:ring-4 focus:ring-slate-950/5 focus:bg-white transition-all text-slate-950 font-bold text-sm" placeholder="e.g. North Kerala">
              </div>
            </div>
          </div>
          
          <button type="button" onclick="goToStep(3, 4)" class="w-full bg-slate-950 text-white py-3 mt-4 rounded-xl font-bold text-sm hover:bg-slate-800 hover:scale-[1.02] hover:shadow-lg transition-all duration-300 flex items-center justify-center gap-2 group">
            Continue <i data-lucide="arrow-right" class="w-4 h-4 transition-transform group-hover:translate-x-1"></i>
          </button>
        </div>
        ...
        <div id="step4" class="w-full step-container step-hidden-right absolute top-0 left-0">
          <div class="flex items-center gap-3 mb-4">
          ...
          <div class="bg-red-50/50 rounded-[20px] p-4 shadow-sm border border-red-100">
            ...
            <label class="flex items-start gap-3 cursor-pointer group mt-4 pt-4 border-t border-red-200">
              <input type="checkbox" id="agreeTerms" class="w-5 h-5 rounded border-red-300 text-red-600 focus:ring-red-500 cursor-pointer mt-0.5" onchange="toggleSubmitButton()">
  ...
  <script>
    ...
    function goToStep(current, next) {
      ...
      stepText.innerText = "Step " + next + " of 4";
    ...
    function showFieldError(inputEl, message) {
      ...
      if (!errBox || !errBox.classList.contains('custom-error-box')) {
        ...
        errBox.className = 'custom-error-box hidden opacity-0 translate-y-1 transition-all duration-300 flex items-center gap-2 text-red-600 bg-red-50 p-2.5 rounded-lg border border-red-100 text-xs font-bold mt-1';
...
console.log('Signup refactored successfully.');
