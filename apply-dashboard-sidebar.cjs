...
const mainContentStartIdx = content.indexOf('<!-- Main Content Area -->');
if (mainContentStartIdx === -1) {
  console.log("Could not find Main Content Area comment.");
  process.exit(1);
}

// Extract everything before the main content
const headerBlock = content.substring(0, mainContentStartIdx);

// Build the new Main Content with Right Sidebar, FAB, Filters, 3-Image Cards, and Modals
const newBody = `<!-- Main Content Area with Right Sidebar Layout -->
  <div class="flex relative">
    
    <!-- Left Main Content -->
    <div class="mt-[80px] md:mt-[90px] px-4 sm:px-6 md:pl-12 md:pr-[290px] py-8 pb-32 md:pb-12 md:h-[calc(100vh-90px)] md:overflow-y-auto hide-scrollbar custom-scrollbar w-full relative z-0">
      
      <!-- Property Status Quick Filters -->
      <div class="flex items-center gap-3 mb-6 overflow-x-auto hide-scrollbar pb-2">
        <button class="bg-red-600 text-white px-5 py-2.5 rounded-full text-sm font-bold shadow-md whitespace-nowrap">All Properties</button>
        <button class="bg-white border border-gray-200 text-slate-700 hover:border-green-500 hover:text-green-600 px-5 py-2.5 rounded-full text-sm font-bold shadow-sm whitespace-nowrap transition-colors flex items-center gap-2">
          <div class="w-2 h-2 rounded-full bg-green-500"></div> Approved Properties
        </button>
        <button class="bg-white border border-gray-200 text-slate-700 hover:border-amber-500 hover:text-amber-600 px-5 py-2.5 rounded-full text-sm font-bold shadow-sm whitespace-nowrap transition-colors flex items-center gap-2">
          <div class="w-2 h-2 rounded-full bg-amber-500"></div> Unapproved Properties
        </button>
      </div>

      <!-- Advanced Search & Filters -->
      <div class="bg-white rounded-[24px] p-4 shadow-sm border border-gray-100 mb-8 flex flex-col xl:flex-row items-center gap-4 relative z-10">
      ...
      <div class="grid grid-cols-[repeat(auto-fit,minmax(350px,1fr))] gap-8">
        ...
        <div class="bg-white rounded-[24px] shadow-sm border border-gray-100 overflow-hidden flex flex-col group relative cursor-pointer hover:shadow-xl transition-all duration-300" onclick="openPropertyModal()">
          <div class="relative w-full h-[260px] overflow-hidden rounded-t-[24px]">
            
            <!-- Mobile Single Image -->
            <img src="https://images.unsplash.com/photo-1600596542815-ffad4c1539a9?ixlib=rb-4.0.3&auto=format&fit=crop&w=800&q=80" class="w-full h-full object-cover group-hover:scale-110 transition-transform duration-700 md:hidden block">
            
            <!-- Desktop 3-Image Gallery -->
            <div class="hidden md:grid grid-cols-3 gap-1 w-full h-full bg-gray-100">
              <div class="col-span-2 overflow-hidden h-full relative">
              ...
              <div class="grid grid-rows-2 gap-1 h-full">
                <div class="overflow-hidden relative">
                  <img src="https://images.unsplash.com/photo-1512917774080-9991f1c4c750?ixlib=rb-4.0.3&auto=format&fit=crop&w=400&q=80" class="w-full h-full object-cover group-hover:scale-110 transition-transform duration-700">
                </div>
                <div class="overflow-hidden relative">
                  <img src="https://images.unsplash.com/photo-1600585154340-be6161a56a0c?ixlib=rb-4.0.3&auto=format&fit=crop&w=400&q=80" class="w-full h-full object-cover group-hover:scale-110 transition-transform duration-700">
                </div>
              </div>
            </div>
            ...
            <div class="absolute top-4 left-4 bg-[#0a192f]/95 backdrop-blur-md text-white px-3 py-1.5 rounded-lg text-[11px] font-bold tracking-wide shadow-sm uppercase border border-white/10">Residential</div>
            <div class="absolute top-4 right-4 bg-[#dc2626]/95 backdrop-blur-md text-white px-3 py-1.5 rounded-lg text-[11px] font-bold tracking-wide shadow-sm uppercase border border-white/10">Sell</div>
            <div class="absolute bottom-4 left-4 bg-[#0a192f]/95 backdrop-blur-md text-white px-3 py-1.5 rounded-lg text-[11px] font-bold tracking-wide shadow-sm uppercase border border-white/10">Apartment</div>
          </div>
          
          <div class="p-6 flex flex-col flex-1 bg-white">
            <div class="flex items-center justify-between mb-1">
              <h3 class="font-extrabold text-xl text-slate-900 line-clamp-1">Marina One - R2S00000177</h3>
              <div class="w-2 h-2 rounded-full bg-green-500 shrink-0 shadow-[0_0_8px_rgba(34,197,94,0.6)]" title="Approved"></div>
            </div>
            ...
            <div class="flex items-center justify-between mt-auto">
              ...
              <button class="bg-[#dc2626] hover:bg-red-700 text-white px-5 py-2.5 rounded-xl text-sm font-bold transition-all shadow-[0_4px_15px_rgba(220,38,38,0.3)] group-hover:shadow-[0_8px_20px_rgba(220,38,38,0.4)] group-hover:-translate-y-0.5 whitespace-nowrap">View property</button>
            </div>
          </div>
        </div>
    ...
    <aside class="hidden md:flex flex-col fixed top-[90px] right-0 w-[270px] h-[calc(100vh-90px)] bg-white border-l border-gray-100 shadow-[-10px_0_30px_rgba(0,0,0,0.02)] z-[100]">
      <div class="p-6 flex flex-col h-full overflow-y-auto hide-scrollbar custom-scrollbar">
        
        <!-- Main Links -->
        <div class="flex flex-col gap-2">
          <a href="/dashboard.html" class="flex items-center gap-4 px-4 py-3 rounded-2xl bg-red-50 text-red-600 font-bold transition-colors">
            <i data-lucide="home" class="w-5 h-5"></i> Dashboard
          </a>
          <a href="#" class="flex items-center gap-4 px-4 py-3 rounded-2xl text-slate-600 font-bold hover:bg-gray-50 hover:text-slate-950 transition-colors">
            <i data-lucide="compass" class="w-5 h-5 opacity-80"></i> Discover
          </a>
          <a href="/properties.html" class="flex items-center gap-4 px-4 py-3 rounded-2xl text-slate-600 font-bold hover:bg-gray-50 hover:text-slate-950 transition-colors">
          ...
          <button onclick="toggleEditProfile()" class="flex items-center gap-4 px-4 py-3 rounded-2xl text-slate-600 font-bold hover:bg-gray-50 hover:text-slate-950 transition-colors text-left">
            <i data-lucide="user" class="w-5 h-5 opacity-80"></i> My Profile
          </button>
        </div>

        <div class="mt-auto pt-8">
          <h4 class="text-xs font-bold text-gray-400 uppercase tracking-wider mb-2 px-4">Other</h4>
          <div class="flex flex-col gap-2">
            ...
            <a href="/login.html" class="flex items-center gap-4 px-4 py-3 rounded-2xl text-red-500 font-bold hover:bg-red-50 transition-colors">
  ...
  <a href="/add-property.html" class="md:hidden fixed bottom-6 right-6 w-14 h-14 bg-red-600 rounded-full flex items-center justify-center text-white shadow-[0_8px_25px_rgba(220,38,38,0.5)] z-[800] hover:scale-110 active:scale-95 transition-transform">
  ...
  <div id="tcModal" class="fixed inset-0 z-[1200] flex items-center justify-center p-4 sm:p-6 opacity-0 pointer-events-none transition-opacity duration-300">
    ...
    <div class="relative bg-white w-full max-w-2xl rounded-[32px] shadow-2xl flex flex-col overflow-hidden transform scale-95 transition-transform duration-500 ease-[cubic-bezier(0.16,1,0.3,1)]" id="tcModalContent">
      ...
      <div class="p-6 overflow-y-auto max-h-[60vh] custom-scrollbar text-sm font-medium text-slate-700 leading-relaxed space-y-4">
        ...
        <h4 class="font-bold text-slate-950 text-lg mt-4 mb-2">2. Verification & Status</h4>
        <p>Properties marked as "Approved" have passed our internal vetting. Unapproved properties remain visible only to you until cleared.</p>
      </div>
  ...
  <script>
    ...
    // Check if lucide exists, create icons after dom load
...
console.log('Applied Right Sidebar, Mobile FAB, and Layout Updates to dashboard.html');
